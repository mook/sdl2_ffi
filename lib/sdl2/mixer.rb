...
module SDL2
  ...
  module Mixer
    ...
    #   open_audio!(frequency, format, channels, chunksize)
    #
    api :Mix_OpenAudio, [:int, :uint16, :int, :int], :int
    ...
    api :Mix_LoadWAV_RW, [RWops.by_ref], Chunk.auto_ptr
    ...
    # :call-seq:
    #   load_mus(filepath)
    api :Mix_LoadMUS, [:string], Music.auto_ptr
    ...
    def self.play_channel(channel,chunk,loops)
    ...
    ##
    # 
    api :Mix_PlayMusic, [Music.by_ref, :int], :int
    ...
    api :Mix_HaltChannel, [:int], :int
    api :Mix_HaltGroup, [:int], :int
    api :Mix_HaltMusic, [:int], :int
